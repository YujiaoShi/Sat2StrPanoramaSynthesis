    ...
    for decoder_layer, (out_channels, dropout) in enumerate(layer_specs):
        ...
        with tf.variable_scope("decoder_%d" % (skip_layer + 1)):
            if decoder_layer == 0:
                ...
                batch, height, width, channel = tf_shape(layers[-1], rank=4)

                input = tf.concat([layers[-1], tf.reshape(layers[skip_layer - 1], [batch, height, width, channel])],
                                  axis=3)
...
def create_model(inputs, targets, ref_images, a):
    ...
    with tf.variable_scope("generator"):
        ...
        with tf.variable_scope('height_estimation'):
            ...
            estimated_height = softargmax(estimated_height)
            # print("*******************************",estimated_height.get_shape().as_list())

        generator_inputs = tf.concat([inputs, estimated_height], axis=-1)
