...
def load_examples(mode='train', batch_size=2):
    ...
    if mode=='train':
        ...
        aer_seman_list = [img_root + 'satseman/' + item[1] for item in trainList if
                    item[0] in exist_grd_list and item[1] in exist_aer_list]
        tanpolar_list = [img_root + 'tanpolarmap/' + item[1] for item in trainList if
                    item[0] in exist_grd_list and item[1] in exist_aer_list]
        ...
        aer_seman_list = [img_root + 'satseman/' + item[1] for item in valList if
                    item[0] in exist_grd_list and item[1] in exist_aer_list]
        tanpolar_list = [img_root + 'tanpolarmap/' + item[1] for item in valList if
                    item[0] in exist_grd_list and item[1] in exist_aer_list]
    ...
    aer_batch, panos_batch, panos_seman_batch, aer_paths_batch, tanpolar_batch, polar_batch = \
        tf.train.batch([aer, panos, panos_seman, aer_paths, tanpolar, polar], batch_size=batch_size)
